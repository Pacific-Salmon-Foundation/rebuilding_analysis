...
esc.data<-nuseds.clean%>%mutate(popid.link=as.character(POP_ID))%>%
  filter(SPP=="CN"&popid.link%in%esc.systems)
...
ggplot(esc.data,aes(x=year,y=escapement))+
  ...
  theme_bw()+
  facet_wrap(~SYS_NM,ncol=3,scales="free_y")+
  theme(strip.background = element_blank())

ggsave("rebuilding spawners.pdf",dpi=600,height=25,width=7)
#write.csv(esc.data,"escapement data from nuseds.csv")
...
ggsave(paste0("rebuilding releases.pdf"),dpi=600,height=15,width=7)
...
pni.data<-read_excel("data/CN PNI DATA from SEP.xlsx",sheet="Sheet1")

pni.data.clean<-pni.data%>%select(Year=`Return Year`,Region,Population,phos.cwt=`pHOS (CWT)`,pnob.cwt=`pNOB (CWT)`,pni.cwt=`PNI (CWT, incl. jacks)`,
                                  phos.thermal=`pHOS (Thermal)`,pnob.thermal=`pNOB (Thermal)`,pni.thermal=`PNI (Thermal)`)

names(pni.data)

sort(unique(pni.data$Population))
